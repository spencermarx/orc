...
Provide a hierarchical, tree-structured popup for navigating the orc tmux session at scale. Complements the command palette (flat, action-oriented) with a spatially-organized view that mirrors the orc hierarchy: project → goal → engineer.
