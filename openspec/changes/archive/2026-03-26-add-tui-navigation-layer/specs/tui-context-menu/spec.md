...
Provide role-aware contextual action menus that surface the right orc operations for each pane type. Menus are organized into safety tiers: navigation (always safe), orchestration (validated before dispatch), and destructive (excluded in v1). The menu itself is never dangerous — all risk is managed by callback validation.
