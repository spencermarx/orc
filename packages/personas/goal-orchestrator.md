...
**CRITICAL — sub-agents must run in YOUR worktree.** The planner must inherit your current working directory. Do NOT use any agent CLI feature that creates a separate git worktree or isolated checkout for the sub-agent — orc already manages worktree isolation, and a second worktree on the same branch will cause git to remove yours (git enforces one worktree per branch). The sub-agent runs in your directory, creates files there, and returns.
...
**Sub-agents must run in your worktree** — do not use any agent CLI feature that creates a separate checkout or worktree for the sub-agent. Same rule as with the planner.
