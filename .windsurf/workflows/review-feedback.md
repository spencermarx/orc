---
description: Review a given code review markdown DOCUMENT or github code review comment or inline feedback
---
...
- **REVIEW SOURCE** (one of the following):
  - A `@[path/to/review.md]` file containing the code review or feedback, OR
  - A GitHub PR link (e.g., `https://github.com/org/repo/pull/123`) pointing to a review or comment, OR
...
1. **Resolve Inputs**
   - **REVIEW SOURCE**: Determine the feedback source:
     ...
     2. If the user provided a GitHub PR link AND it is not already present in the chat history, use `read_url_content` to fetch the review content. If the link points to a specific comment, focus on that comment; otherwise, gather all review comments.
